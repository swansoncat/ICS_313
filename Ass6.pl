/* This is the file containing the prolog DCG for assignment 6. It is an extension of the assignment 5 file we were given on laulima.
It is able to answer the queries given in the homework instructions, and the write command doesn't just print out ascii characters anymore,
but I was unable to implement testing for ungrammatical queries.


Note to self: all that you had to do to change the write() from printing ascii values to text was change the double quotes to single quotes.
*/
...
proper_noun(keisha_castle_hughes) -->[keisha, castle, hughes].
...
proper_noun(billy_dee_williams) -->[billy, dee, williams].
...
directed(richard_marquand, star_wars5).
...
acts_in(billy_dee_williams, star_wars5).
...
acts_in(alec_guiness, star_wars5).
...
play(liam_neeson, qui_gon_jinn).
play(jake_lloyd, anakin_skywalker).
play(pernilla_august, shmi_skywalker).
play(oliver_ford_davies, sio_bibble).
play(hugh_quarshie, captain_panaka).
play(ahmed_best, jar_jar_binks).

play(temuera_morrison, jango_fett).
...
play(billy_dee_williams, lando_calrissian).
...
play_as(liam_neeson, qui_gon_jinn, star_wars1).
play_as(ewan_mcgregor, obi_wan_kenobi, star_wars1).
play_as(natalie_portman, padme, star_wars1).
play_as(jake_lloyd, anakin_skywalker, star_wars1).
play_as(ian_mcdiarmid, palpatine, star_wars1).
play_as(pernilla_august, shmi_skywalker, star_wars1).
play_as(oliver_ford_davies, sio_bibble, star_wars1).
play_as(hugh_quarshie, captain_panaka, star_wars1).
play_as(ahmed_best, jar_jar_binks, star_wars1).
play_as(anthony_daniels, c3p0, star_wars1).

play_as(ewan_mcgregor, obi_wan_kenobi, star_wars2).
play_as(natalie_portman, padme, star_wars2).
play_as(hayden_christensen, anakin_skywalker, star_wars2).
play_as(christopher_lee, count_dooku, star_wars2).
play_as(samuel_jackson, mace_windu, star_wars2).
play_as(frank_oz, yoda, star_wars2).
play_as(ian_mcdiarmid, palpatine, star_wars2).
play_as(pernilla_august, shmi_skywalker, star_wars2).
play_as(temuera_morrison, jango_fett, star_wars2).
play_as(jimmy_smits, bail_organa, star_wars2).
...
play_as(mark_hamill, luke_skywalker, star_wars5).
play_as(harrison_ford, han_solo, star_wars5).
play_as(carrie_fisher, leia_organa, star_wars5).
play_as(billy_dee_williams, lando_calrissian, star_wars5).
play_as(anthony_daniels, c3p0, star_wars5).
play_as(david_prowse, darth_vader, star_wars5).
play_as(peter_mayhew, chewbacca, star_wars5).
play_as(kenny_baker, r2d2, star_wars5).
play_as(frank_oz, yoda, star_wars5).
play_as(alec_guiness, obi_wan_kenobi, star_wars5).
...
top([what,is,the,title,for,star,wars,iii]).

top([what,is,the,title,for,star,wars,iv]).
